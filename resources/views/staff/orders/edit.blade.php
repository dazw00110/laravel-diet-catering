...
<div class="max-w-5xl mx-auto bg-white shadow-lg p-10 rounded-xl" x-data="orderForm()">
    ...
    <form method="POST" action="{{ route('staff.orders.update', $order->id) }}" class="space-y-6">
        @csrf
        @method('PUT')
...
<input class="input input-bordered w-full bg-gray-100" disabled value="{{ $statusLabels[$order->status] ?? $order->status }}">
            </div>
            <div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
        ...
        <div class="mt-4 text-sm text-gray-700">
            <p class="font-semibold mb-1">🎁 Dostępne kody rabatowe:</p>
            <ul class="list-disc pl-5 mb-3">
                @foreach ($discounts as $discount)
                    <li>{{ $discount->code }} — {{ $discount->type === 'percentage' ? $discount->value . '%' : $discount->value . ' zł' }}</li>
                @endforeach
            </ul>

            <p class="font-semibold mb-1">📢 Inne promocje:</p>
            <ul class="list-disc pl-5">
                <li>4+1 gratis (przy ≥ 5 sztuk produktu)</li>
                <li>-10% za zamówienie powyżej 2000 zł</li>
                <li>-15% za zamówienie powyżej 3000 zł</li>
                <li>-5% za 3+ zamówienia w tygodniu</li>
            </ul>
        </div>
        ...
        <button type="submit" class="btn bg-green-600 text-white w-full mt-6">💾 Zapisz zmiany</button>
    </form>
</div>
