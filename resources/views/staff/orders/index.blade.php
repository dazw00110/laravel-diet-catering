    ...
    <tbody>
        @forelse ($orders as $order)
            ...
            <tr>
                ...
                <td class="p-2 border-b text-center">{{ \Illuminate\Support\Carbon::parse($order->start_date)->format('Y-m-d') }}</td>
                <td class="p-2 border-b text-center">{{ \Illuminate\Support\Carbon::parse($order->end_date)->format('Y-m-d') }}</td>
                <td class="p-2 border-b text-center">{{ $location }}</td>
                <td class="p-2 border-b text-center space-y-1 space-x-1">
                    <a href="{{ route('staff.orders.show', $order) }}" class="bg-blue-500 text-white px-2 py-1 rounded text-sm hover:bg-blue-600">Pokaż</a>
                    <a href="{{ route('staff.orders.edit', $order) }}" class="bg-yellow-400 text-white px-2 py-1 rounded text-sm hover:bg-yellow-500">Edytuj</a>
                    <form action="{{ route('staff.orders.destroy', $order) }}" method="POST" class="inline">@csrf @method('DELETE')
                    ...
                    <form action="{{ route('staff.orders.cancel', $order) }}" method="POST" class="inline">@csrf
                        <button
                    ...
                    <form action="{{ route('staff.orders.complete', $order) }}" method="POST" class="inline">@csrf
                        <button
                            @if ($order->status !== 'in_progress')
